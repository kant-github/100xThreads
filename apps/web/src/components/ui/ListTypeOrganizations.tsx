            ...
            {organizations.map((organization, index) => {
                ...
                return (
                    <Link href={`/org/${organization.id}`}
                        className="w-full flex flex-col justify-between gap-y-4 md:gap-y-6 lg:gap-y-8 items-between px-4 md:px-6 lg:px-12 py-4 border-b-[0.5px] border-zinc-700 h-36 dark:bg-terDark" key={index}>
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-y-3 sm:gap-y-0">
                            <div className="flex flex-row items-center gap-x-3 w-full sm:w-auto">
                                <Image
                                    className="rounded-[14px] w-8 h-8 md:w-12 md:h-12 border-[1px] dark:border-neutral-700"
                                    width={50}
                                    height={40}
