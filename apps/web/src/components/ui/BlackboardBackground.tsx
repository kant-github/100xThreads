interface BlackboardBackgroundProps {
    children: React.ReactNode
    className?: string
}

export default function ({ children, className }: BlackboardBackgroundProps) {
    return (
        <div className={`relative flex-grow overflow-hidden mt-4 rounded-[8px] ${className}`}>
            <div className="absolute inset-0 bg-[#171717] before:absolute before:inset-0 before:bg-[radial-gradient(circle_at_center,_rgba(255,255,255,0.1)_0%,_transparent_10%)] before:bg-repeat before:[background-size:8px_8px] before:opacity-50"></div>
            <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-[0.15]"></div>
            <div className="relative">
                <div className="text-slate-100 font-chalk">
                    {children}
                </div>
            </div>
        </div>
    )
}